...
int calc_drone_pos(int force, int firstCoordinate, int secondCoordinate)
{
    double x;
...
double calculateAngle(struct Coordinates *currentCoordinates, struct Coordinates *obstaclePos)
{
    double angleRadians = atan2(obstaclePos->y - currentCoordinates->y, obstaclePos->x - currentCoordinates->x);
    return angleRadians;
}

struct Coordinates calc_repulsive(struct Coordinates obstaclePos[], int numberOfObstacles, struct Coordinates *currentCoordinates, struct Forces *Forces)
{
...
void update_pos(struct Forces *Forces, struct Coordinates *currentCoordinates, struct Coordinates *previousCoordinates, struct Coordinates *obstaclePos, int numberOfObstacles)
{
    struct Coordinates repulsion;
...
int main(int argc, char *argv[])
{
    ...
    do
    {
        char direction = '9';
        isMoving = 1;
        ssize_t bytesRead = read(pipeID1, &message, sizeof(message)); // Receive command force from keyboard_manager
